...
def test_get_podcast_by_page(in_memory_repo):
    dict1 = browse_services.get_podcasts_by_page(in_memory_repo, 1)
    assert len(dict1['podcasts']) == 4
    assert repr(dict1['podcasts'][3]) == "<Podcast 4: 'Tallin Messages' by Tallin Country Church>"
    assert repr(dict1['podcasts'][2]) == "<Podcast 3: 'Onde Road - Radio Popolare' by Brian Denny>"
    assert dict1['podcasts'][2].itunes_id == 568005832


def test_get_podcast_by_category(in_memory_repo):
    dict0 = browse_services.get_podcasts_by_category(in_memory_repo, 'Society & Culture', 1)
    assert len(dict0['podcasts']) == 2
    assert dict0['current_page'] == 1
    assert dict0['has_next'] is False
    assert dict0['has_previous'] is False
    assert dict0['next_page'] == 1
    assert dict0['previous_page'] == 1

    dict1 = browse_services.get_podcasts_by_category(in_memory_repo, 'Professional', 1)
    assert len(dict1['podcasts']) == 2
    assert dict1['current_page'] == 1
...
def test_get_random_podcasts_info(in_memory_repo):
    dict0= home_services.get_random_podcasts_info(in_memory_repo, 3)
    assert len(dict0) == 3
...
def test_remove_from_playlist(mock_get_username, in_memory_repo):
    ...
    assert "<Playlist 1 'Test_User3's Playlist': Owned by test_user3>" == repr(
        description_services.get_playlist(in_memory_repo))
...
def test_add_review(in_memory_repo):
    ...
    assert repr(user.reviews[
                    0]) == "<Review 1 made by Jill for podcast 'The rise of the phoenix' with a rating of 5 and a description of Wonderful and insightful podcast>"
...
def test_get_podcasts_filtered_by_category(in_memory_repo):
    ...
    assert "<Podcast 4: 'Tallin Messages' by Tallin Country Church>" == repr(
